/*
 * @Author: xiaoran
 * @Date: 2023-10-06 16:30:44
 * @LastEditTime: 2023-10-08 12:53:09
 * @Description: 单链表实现
 */
...
Status GetElem (LinkList L, int i, ElemType *e)
{
    ...
    if (!p || j>i)
        return ERROR;    // 第i个元素不存在（可以在头结点数据域存储链表长度，避免如此）
...
// 初始条件：单链表L已存在，e为要查找的元素
// 操作结果：查找成功则返回序号（从1开始，头结点不算），否则返回0表示失败
int LocateElem (LinkList *L, ElemType e)
{
    LinkList p;
    p = (*L)->next;    // 指向单链表头结点下一个结点
...
// 初始条件：单链表L已存在
// 操作结果：返回单链表L的元素个数（不算头结点）
int ListLength (LinkList *L)
{
    LinkList p;
    p = (*L)->next;    // 指向单链表头结点的下一个结点
